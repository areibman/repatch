...
import type { RepoStats, GitHubCommit } from "./types";
import { getCommitsForFilters } from "./commit-filters";
import { fetchCommitStats } from "./resource-fetchers";
...
export async function getRepoStats(
  ...
  const estimationFactor =
    commitsToFetch.length > 0 ? commits.length / commitsToFetch.length : 1;
