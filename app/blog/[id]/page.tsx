...
export default function BlogViewPage() {
  ...
  const [isQueuingThread, setIsQueuingThread] = useState(false);
  ...
  const handleQueueTwitterThread = async () => {
    if (!patchNote) return;

    const includeVideo = confirm(
      'Do you want to include a video with this Twitter thread?\n\n' +
      'Note: Video rendering and upload may take a few minutes.'
    );

    setIsQueuingThread(true);
    try {
      const response = await fetch('/api/integrations/typefully/queue', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to queue Twitter thread');
      }

      const data = await response.json();
      alert(
        `✅ Twitter thread queued successfully!\n\n` +
        `Thread ID: ${data.threadId}\n\n` +
        `You can view and edit it in your Typefully dashboard.`
      );
    } catch (error) {
      console.error('Error queueing Twitter thread:', error);
      const errorMessage = error instanceof Error ? error.message : 'Failed to queue Twitter thread';
      alert(`❌ Error: ${errorMessage}`);
    } finally {
      setIsQueuingThread(false);
    }
  };
              ...
              {isEditing ? (
                ...
                <>
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={handleQueueTwitterThread}
                    disabled={isQueuingThread}
                  >
                    {isQueuingThread ? (
                      <>
                        <Loader2Icon className="h-4 w-4 mr-2 animate-spin" />
