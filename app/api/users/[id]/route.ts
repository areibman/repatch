...
export async function GET(
  ...
  const result = await getUser(actor.userId, params.id);
...
export async function PATCH(
  ...
  const result = await updateUser(actor.userId, params.id, payload as Record<string, never>);
  return fromServiceResult(result);
}
...
export async function DELETE(
  ...
  const result = await deactivateUser(actor.userId, params.id);
  return fromServiceResult(result);
}
