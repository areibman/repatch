...
<body>
  <div class="container">
    ...
    const sendResult = await provider.sendPatchNoteEmail({
    ...
    return NextResponse.json({
      success: true,
      provider: summarizeIntegration(provider.config),
      sentTo: sendResult.totalRecipients,
      messageIds: sendResult.providerMessageIds,
    });
  } catch (error) {
